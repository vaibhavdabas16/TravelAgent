...
export function EntertainmentSection({ planningData, onSelectionChange, isTransitioning }: EntertainmentSectionProps) {
  ...
  const getTypeColor = (type: string) => {
    const colors = {
      theater: 'bg-brand/20 text-purple-300',
      concert: 'bg-brand/20 text-blue-300',
    ...
    <div className="max-w-6xl mx-auto">
      ...
      {sortedEntertainment.length === 0 ? (
        <div className="text-center py-12 bg-surface rounded-xl border border-line">
          <Music className="w-12 h-12 text-ink/20 mx-auto mb-4" />
          <h3 className="text-xl text-ink mb-2">No entertainment found</h3>
          <p className="text-ink-subtle">We couldn't find any specific entertainment options for this destination.</p>
        </div>
      ) : (
                ...
                <Card className={`overflow-hidden cursor-pointer transition-all duration-300 ${isSelected
                  ...
                  <div className="p-5">
                    <div className="flex justify-between items-start mb-3">
                      <h3 className="text-ink text-lg leading-tight">{entertainment.name}</h3>
                    ...
                    <div className="flex justify-between items-center">
                      <span className="text-brand">{entertainment.price}</span>
                      <Button
                        size="sm"
